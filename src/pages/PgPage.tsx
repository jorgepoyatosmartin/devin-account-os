...
function PgDashboard({ dataset, upsertPg, addPg, addOpportunity, onToast }: Props) {
  ...
  return <>
    <PageHeader eyebrow="PIPELINE GENERATION / PG" title={t('pg.title')} subtitle={t('pg.subtitle')} action={<button className="button primary" onClick={() => setShowComposer((value) => !value)}>{t('pg.newTarget')}</button>} />
...
function PgComposer({ dataset, initialStakeholderId, addPg }: { dataset: Dataset; initialStakeholderId: string; addPg: (record: PgRecord) => void }) {
  ...
  return <Section title={t('pg.composer')} eyebrow="COMPOSER"><form className="pg-composer" onSubmit={create}>
