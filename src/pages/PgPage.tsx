  ...
  return <>
    ...
    <Section title={t('pg.todayPriorities')} eyebrow={t('pg.topNextMoves')}>
      <div className="pg-priority-grid">{prioritiesToday.map((record) => <article className="pg-priority-card" key={record.id} onClick={() => navigate(`/pg/${record.id}`)}>
        ...
        <div className="pg-copy"><span>{t('pg.action')}</span><p>{record.action}</p></div>
...
function PgDetail({ dataset, id, upsertPg, addOpportunity, addTask, onToast }: Props & { id: string }) {
  ...
  return <>
    ...
    <Section title={t('pg.detail.whyHigh')} eyebrow={t('pg.qualification')}><div className="auto-line"><Badge>{t('pg.detail.auto')}</Badge>{view.whyHighTarget}</div><textarea value={raw.whyHighTarget} placeholder={t('pg.autoInference')} onChange={(e) => set({ whyHighTarget: e.target.value })} /><button className="button secondary" onClick={() => set({ whyHighTarget: '' })}>{t('pg.detail.resetAuto')}</button></Section>
    <Section title={t('pg.detail.whyMeet')} eyebrow={t('pg.qualification')}><div className="auto-line"><Badge>{t('pg.detail.auto')}</Badge>{view.whyMeet}</div><textarea value={raw.whyMeet} placeholder={t('pg.autoInference')} onChange={(e) => set({ whyMeet: e.target.value })} /><button className="button secondary" onClick={() => set({ whyMeet: '' })}>{t('pg.detail.resetAuto')}</button></Section>
    <Section title={t('pg.detail.howMeeting')} eyebrow={t('pg.access')}><div className="auto-line"><Badge>{t('pg.detail.auto')}</Badge>{view.howGetMeeting}</div><select value={view.accessRoute} onChange={(e) => set({ accessRoute: e.target.value as PgRecord['accessRoute'] })}>{['Direct outreach', 'Internal introduction', 'Champion introduction', 'Executive introduction', 'Partner introduction', 'Event', 'Existing opportunity', 'Existing customer relationship'].map((x) => <option key={x}>{tv(x)}</option>)}</select></Section>
    <Section title={t('pg.detail.message')} eyebrow={t('pg.outreach')}><div className="language-toggle"><button className={language === 'es' ? 'selected' : ''} onClick={() => setLanguage('es')}>ES</button><button className={language === 'en' ? 'selected' : ''} onClick={() => setLanguage('en')}>EN</button><button className="button secondary" onClick={generate}>{t('pg.detail.generate')}</button><button className="button secondary" onClick={async () => { try { await navigator.clipboard?.writeText(view.message); onToast(t('toast.messageCopied')); } catch { onToast(t('pg.messageReady')); } }}>{t('pg.detail.copy')}</button><button className="button primary" onClick={markSent}>{t('pg.detail.markSent')}</button></div><textarea value={raw.message} placeholder={t('pg.detail.generatePlaceholder')} onChange={(e) => set({ message: e.target.value, messageLang: language })} /></Section>
    <Section title={t('pg.detail.action')} eyebrow={t('pg.nextMove')}><div className="auto-line"><Badge>{t('pg.detail.auto')}</Badge>{view.action}</div><input value={raw.action} placeholder={t('pg.autoInference')} onChange={(e) => set({ action: e.target.value })} /><button className="button secondary" onClick={() => { addTask({ id: `task-${Date.now()}`, title: view.action, accountId: view.accountId, stakeholderId: view.stakeholderId, status: 'Open', createdAt: new Date().toISOString(), source: 'pg' }); onToast(t('toast.actionAdded')); }}>{t('pg.detail.addTask')}</button></Section>
